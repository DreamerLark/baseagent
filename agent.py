...
class BaseAgent:
    ...
    async def register_mcp_server(self, name: str, command: List[str], timeout: int = 30, env: Optional[Dict[str, str]] = None):
        """
        ...
        """
        await self.mcp_manager.add_server(name, command, timeout, env)
        
    async def register_mcp_servers_from_config(self, config: Dict[str, Any]):
        """
        Register multiple MCP servers from a configuration object
        
        Args:
            config: Configuration object in the format:
                {
                    "server_name": {
        ...
        for server_name, server_config in config.items():
            ...
            if not command:
                print(f"Warning: Skipping server '{server_name}' - no command specified")
                continue
                
            try:
                await self.register_mcp_server(server_name, command, timeout, env)
                print(f"Registered MCP server: {server_name}")
            except Exception as e:
                print(f"Failed to register MCP server '{server_name}': {str(e)}")
                
    async def register_mcp_servers_from_file(self, file_path: str):
        """
        Register MCP servers from a JSON or JSONC file
        
        Args:
            file_path: Path to JSON/JSONC file containing mcpServers configuration
        """
        try:
            # Handle JSONC files by removing comments (lines starting with // or /* */)
...
, '', content, flags=re.MULTILINE)
            ...
            if "mcpServers" in config_data:
                ...
                # Assume the whole file is mcpServers config
                mcp_servers = config_data
                
            await self.register_mcp_servers_from_config(mcp_servers)
            
        except FileNotFoundError:
            print(f"Configuration file not found: {file_path}")
        except json.JSONDecodeError as e:
            print(f"Invalid JSON in configuration file {file_path}: {str(e)}")
        except Exception as e:
            print(f"Failed to load MCP configuration from {file_path}: {str(e)}")
